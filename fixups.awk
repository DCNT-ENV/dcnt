...
# $0~"github.com/DCNT-Hammer/dcnt/util/atomic" {inimport=0} # don't duplicate the atomic import
# FILENAME~/(atomic)|(rateCalculator)/ && inimport {inimport=0} # don't import atomic in these files that use sync on purpose
# /^)$/ && inimport {inimport=0; 	print "\"github.com/DCNT-Hammer/dcnt/util/atomic\"";}# add atomic to import, goimport will toss it if it's not needed
