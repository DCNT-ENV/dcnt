#!/bin/bash
echo "run this from DCNT-Hammer/dcnt eg:"
...
rm "$GOPATH/bin/dcnt"
echo "Compiling..."
go install -ldflags "-X github.com/DCNT-Hammer/dcnt/engine.Build=`git rev-parse HEAD` -X github.com/DCNT-Hammer/dcnt/engine.dcntVersion=`cat VERSION`"
if [ $? -eq 0 ]; then
     echo "was binary updated? Current:`date`"
    ls -G -lh "$GOPATH/bin/dcnt"

    echo "changing directory to dcnt/p2p"
    cd "$GOPATH/src/github.com/DCNT-Hammer/dcnt/p2p"
    pkill dcnt
